...
/**
 * Contiene sistema LJ junto con su valor de libertad y cambio de energía.
 */
class Individuo {
...
public class LJParticlesApp extends AbstractSimulation {
	...
	*/
	private java.util.List<Individuo> mutacion() {
		...
		for (Individuo i : P.getIndividuos()) {
			...
			md.setVelocitiesEvolution(); // Cambia la velocidad de las partículas con 10% de probabilidad.
	...
	**/
	private void buscarSelection(LJParticles mdI) {
		...
		do {
			// Realiza mutaciones sobre la población P, las añade a la lista de población y evoluciona.
			...
			// Ordena la lista de individuos según su valor de libertad.
	...
	public void stop() {
		...
		control.println("Número de pasos = " + mdII.steps);
		...
		control.println("Capacidad calorífica = " + decimalFormat.format(mdII.getHeatCapacity()));
